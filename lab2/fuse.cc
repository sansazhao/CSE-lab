...
void fuseserver_symlink(fuse_req_t req, const char *path, fuse_ino_t parent, const char *name){
    struct fuse_entry_param e;
    e.attr_timeout = 0.0;
    e.entry_timeout = 0.0;
    e.generation = 0;
    yfs_client::status ret;
    yfs_client::inum ino;
    ret = yfs->symlink(parent, name, path, ino);
    if(ret == yfs_client::OK){
//    if(ret = yfs->symlink(parent, name, path, ino) == yfs_client::OK){
        e.ino = ino;
        getattr(ino, e.attr);
        fuse_reply_entry(req, &e);
    }
...
void fuseserver_readlink(fuse_req_t req, fuse_ino_t ino){
    std::string path;
    yfs_client::status r = yfs->readlink(ino, path);
    if(r == yfs_client::OK){
